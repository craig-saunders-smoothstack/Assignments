...
public class DateTimeAPIExamples {
    ...
    {
        List<LocalDate> thursdays = random_date
                .minus(1, ChronoUnit.WEEKS)
                .datesUntil(random_date)
                .filter(d->d.getDayOfWeek() == DayOfWeek.THURSDAY)
    ...
    public boolean printMondaysInMonthYear(Month m, Integer year)
    {
        ...
        date.datesUntil(
                LocalDate.of(year, m, Year.of(year)
                        .atMonth(m).lengthOfMonth()))
        .filter(d->d.getDayOfWeek() == DayOfWeek.MONDAY)
        .collect(Collectors.toList()).stream().forEach(d-> System.out.println(d));
        return true;
    }
    
    /**
     * @param date : Given LocalDate
     * @return Returns true if date given is Friday and the 13th of the month, else false.
     */
